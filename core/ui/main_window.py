...
from PyQt6.QtGui import QFont, QAction, QKeySequence, QRegularExpressionValidator, QIcon
...
class RestoreDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.tree.setHeaderLabels(["", "Date", "Original Path", "New Path", "Size", "Group"])
        ...
        for root, dirs, files in os.walk(self.parent.state['dupes_folder']):
            if 'move_log.json' in files:
                ...
                try:
                    ...
                    folder_item = QTreeWidgetItem([f"📁 {os.path.basename(root)}", "", "", "", "", ""])
                    folder_item.setExpanded(True)
                    folder_item.setData(0, Qt.ItemDataRole.UserRole, {'type': 'folder', 'path': root})

                    for file_info in log_data.get('files', []):
                        file_item = QTreeWidgetItem([
                            "",
                            file_info.get('timestamp', '')[:10],
                            file_info.get('original_path', ''),
                            file_info.get('new_path', ''),
                            self.parent.format_size(file_info.get('size', 0)),
                            file_info.get('group_hash', '')[:8] + '...'
                        ])
    ...
    def restore_selected(self):
        ...
        reply = QMessageBox.question(
            ...
            "WARNING: If original files still exist, they will be overwritten!",
...
class MainWindow(QMainWindow):
    ...
    def show_shortcuts(self):
        ...
        <tr><td>Restore Files</td><td>Ctrl+Shift+R</td></tr>
